...
export function SettingsPage() {
  ...
  if (!mockModeEnabled) return <UnavailableFeature title="Cấu hình vận hành" />;
  return (
    <>
      <PageHeader
        description="Múi giờ nghiệp vụ Asia/Ho_Chi_Minh • chính sách có phiên bản"
      ...
      <div className="settings-grid">
        <section className="panel">
          ...
          <label>
            Số yêu cầu tối đa/cửa hàng
            <input defaultValue="2" min="1" type="number" />
          </label>
          <label>
            Phiên bản chính sách
            <input defaultValue="ALLOC-v1.2" readOnly />
          </label>
        </section>
        <section className="panel">
          <h2>Tích hợp IDOSI</h2>
          <label>
            Endpoint
            <input
              defaultValue="https://idosi.io.vn/api/integrations/warehouse/v1/order-statistics"
              readOnly
            />
          </label>
          <label>
            Trạng thái secret
            <input defaultValue="Đã cấu hình phía server" readOnly />
          </label>
          <label>
            Chu kỳ tự đồng bộ
          ...
          <div className="conservation-note">
            <ShieldCheck size={17} />
            <span>Secret không bao giờ gửi xuống frontend.</span>
          </div>
        </section>
      </div>
      <div className="sticky-actions">
        <span>
          {saved
            ? 'Đã lưu phiên bản cấu hình mới.'
            : 'Thay đổi tạo phiên bản mới, không sửa lịch sử.'}
        </span>
        <Button onClick={() => setSaved(true)}>Lưu cấu hình</Button>
      </div>
    </>
  );
}
