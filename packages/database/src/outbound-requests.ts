...
export async function dispatchWarehouseOutboundRequest(
  ...
  return withIdempotency(
    ...
    (tx) =>
      withAdvisoryLock(tx, 'warehouse-outbound', input.outboundRequestId, async () => {
        const [outbound] = await tx
          .select()
          .from(outboundRequests)
          .where(
            and(
              eq(outboundRequests.id, input.outboundRequestId),
              isNull(outboundRequests.deletedAt),
            ),
          )
          .for('update')
        ...
        const dispatcherRole = await assertDispatcherMayAccessStore(
          tx,
          input.dispatchedByUserId,
          outbound.storeId,
        );
        ...
        for (const line of lines) {
          await tx
            ...
            .where(
              and(
                eq(outboundRequestLines.id, line.id),
                eq(outboundRequestLines.dispatchedQuantity, 0),
              ),
            );
        }
        ...
        const record = recordFromRows(updated, dispatchedLines);
        await tx.insert(auditLogs).values({
          requestId: input.requestId ?? null,
          actorUserId: input.dispatchedByUserId,
          ...
          metadata: dispatchNote ? { dispatchNote } : {},
        });
...
async function assertDispatcherMayAccessStore(
  ...
  const [user] = await tx
    ...
    .limit(1);
  if (!user || user.status !== 'active' || user.role === 'store') {
