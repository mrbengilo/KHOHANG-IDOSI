...
export async function confirmSupplierInboundCostsInTransaction(
  ...
  return withAdvisoryLock(tx, 'supplier-inbound-receipt', input.receiptId, async () => {
    ...
    // VAT is now entered on store receipts. Only receipts created before that change can still
    // carry a warehouse VAT amount; it stays part of their recorded total.
    const totalCostVnd =
      goodsCostVnd +
